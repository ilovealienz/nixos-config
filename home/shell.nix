...

{
  ...
  programs.zsh = {
    ...
    initContent = ''
      nxedit() { local out key f; while out=$(cd /etc/nixos && fd -e nix | awk -F/ '{ if (NF==1) printf "0\t\033[1;38;2;163;190;140m%s\033[0m\n",$0; else { d=$0; sub("/[^/]*$","",d); n=$0; sub(".*/","",n); printf "1\t\033[38;2;129;161;193m%s/\033[0m\033[38;2;216;222;233m%s\033[0m\n",d,n } }' | sort | cut -f2- | fzf --ansi --reverse --expect=ctrl-r,ctrl-g,ctrl-a --prompt='➜ ' --header 'enter edit · ^r rebuild · ^g diff · ^a git add' --preview 'bat --color=always --style=numbers /etc/nixos/{}' --preview-window 'right,55%,border-left' --color='gutter:-1,hl:#88c0d0,fg+:#eceff4,bg+:#3b4252,hl+:#8fbcbb,pointer:#bf616a,marker:#a3be8c,header:#81a1c1,border:#4c566a,prompt:#81a1c1'); do key=$(head -1 <<< "$out"); f=$(sed -n 2p <<< "$out"); [[ -z "$f" ]] && continue; case "$key" in ctrl-r) nxrebuild ;; ctrl-g) sudo git -C /etc/nixos diff -- "$f" | less -R ;; ctrl-a) sudo git -C /etc/nixos add -f "$f" && echo "staged $f" ;; *) ''${EDITOR:-nvim} "/etc/nixos/$f" ;; esac; done; }
